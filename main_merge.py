...
def loadData_cs(data_path):
    ...
    #Creating a sparse col matrix, use the lowest data type (int8) possible in this case to reduce RAM cost 
    sparse_colMatrix_full = csc_matrix((ratings_full, (movie_ids_full, user_ids_full)),dtype=np.int8)

    #Remove the first row and column of the sparse col matrix, since it adds one too many for each 
    sparse_colMatrix_full = sparse_colMatrix_full[:,1:]
    sparse_colMatrix_full = sparse_colMatrix_full[1:,:]
    
    #Check the amount of users
    print("Number of users is: ", np.shape(sparse_colMatrix_full[0,:])[1])
    print("Number of movies is: ", np.shape(sparse_colMatrix_full)[0])
    return sparse_colMatrix_full
